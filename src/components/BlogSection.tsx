...
const blogPosts = [
  { slug: 'fiverr-passive-income-2026', title: 'How to Turn Fiverr Skills into Passive Income in 2026', excerpt: 'The complete guide to escaping the freelancing trap and building automated income streams.', category: 'Passive Income', readTime: '12 min' },
  ...
  return (
    <section id="blog" className="bg-white py-24 md:py-32">
      <div className="max-w-[1080px] mx-auto px-6">
        ...
        >
          <p className="text-sm font-medium text-brand uppercase tracking-widest mb-4">Free Resources</p>
          <h2 className="text-[32px] md:text-[42px] font-light tracking-[-0.02em] text-navy mb-5">
            In-Depth{' '}
            <span className="bg-gradient-to-r from-brand to-brand-light bg-clip-text text-transparent font-normal">Guides</span>
          </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {blogPosts.map((post, i) => (
            <motion.a
              ...
              className="group bg-white border border-border rounded-xl p-7 hover:shadow-[0_8px_30px_rgba(50,50,93,0.08)] hover:border-brand/20 transition-all duration-300 hover:-translate-y-1 block"
            >
              <div className="flex items-center gap-2 mb-4">
                <span className="text-xs font-medium text-brand bg-brand/10 px-3 py-1 rounded-full">{post.category}</span>
                <span className="text-xs text-slate">{post.readTime}</span>
              </div>
              <h3 className="text-lg font-medium text-navy mb-2 group-hover:text-brand transition-colors">{post.title}</h3>
              <p className="text-slate text-sm mb-4 leading-relaxed">{post.excerpt}</p>
              <span className="text-brand text-sm font-medium group-hover:translate-x-1 inline-block transition-transform">Read more →</span>
            </motion.a>
          ))}
        </div>
      </div>
